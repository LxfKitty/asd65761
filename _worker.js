...
// ===== CFnew UI layer v1.2.0: upstream 2026-08-11 core snapshot + iOS glass UI, unified vector icons, robust scrolling =====
const __CF_UI_VERSION = 'cfnew-ios-glass-edt-v1.2.0';
...
function iconFor(text){var t=(text||'').toLowerCase();if(/当前网络|网络信息|地区|墙外|国内测试/.test(t))return'wifi';if(/节点链接|复制节点|link/.test(t))return t.indexOf('复制')>=0?'copy':'link2';if(/优选|开始优选|小白|简单/.test(t))return'wand';if(/详细|配置|参数|设置|模板/.test(t))return'sliders';if(/ech|encrypted|加密/.test(t))return'keyhole';if(/cloudflare|cdn|proxyip/.test(t))return'cloudbolt';if(/订阅转换/.test(t))return'repeat';if(/刷新|更新/.test(t))return'refresh';if(/重置/.test(t))return'undo';if(/消息|通知|telegram/.test(t))return'bell';if(/日志|清单|列表/.test(t))return'list';if(/二维码/.test(t))return'scan';if(/复制/.test(t))return'copy';if(/保存|确定|应用/.test(t))return'check';if(/退出/.test(t))return'doorout';if(/链式|路由/.test(t))return'branch';if(/获取更多|搜索|验证/.test(t))return'search';if(/socks|安全|证书/.test(t))return'shield';if(/关闭|取消/.test(t))return'xmark';return'help'}
...
function addSemanticIcons(){var sel='.module-title,.section-title,.card-title,.network-card-title,h2,h3,button,a.btn,button[class*="btn"]';document.querySelectorAll(sel).forEach(function(el){if(el.dataset.cfnewIconDone)return;var original=String(el.textContent||'');var raw=stripEmoji(original);if(!raw)return;var hadEmoji=emojiRegex().test(original);var cls=String(el.className||'');var isHeading=/module-title|section-title|card-title|network-card-title/.test(cls)||/^(H2|H3)$/.test(el.tagName);var should=isHeading||hadEmoji||/二维码|复制|保存|退出|重置|参数配置|清除配置|路径模板|获取更多|为什么|开始优选|订阅接口|链式代理|我是小白/.test(raw);if(!should)return;el.dataset.cfnewIconDone='1';el.insertAdjacentHTML('afterbegin',icon(iconFor(raw),'cfnew-inline-icon'))})}
...
function modules(){var enabled=(document.documentElement.getAttribute('data-cfnew-modules')||'').split(',').map(function(x){return x.trim()});if(enabled.indexOf('notifications')<0){var a=findSection('消息通知设置');if(a)a.remove()}if(enabled.indexOf('logs')<0){var b=findSection('查看操作日志');if(b)b.remove()}}
...
function toast(text){var t=document.getElementById('cfnewToast');if(!t){t=document.createElement('div');t.id='cfnewToast';t.className='cfnew-toast';document.body.appendChild(t)}t.textContent=text;t.classList.add('is-show');clearTimeout(t._timer);t._timer=setTimeout(function(){t.classList.remove('is-show')},1800)}
...
function __CFNormalizeModules(value) {
  const allowed = new Set(['notifications', 'logs']);
