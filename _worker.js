...
// ===== CFnew UI layer v1.3.0: upstream 2026-08-11 core snapshot + iOS glass UI, Lucide-style vector icons, cool-tone controls, robust scrolling, passive-notice suppression =====
const __CF_UI_VERSION = 'cfnew-ios-glass-edt-v1.3.0';
...
/* CFnew v1.3: remove upstream amber/yellow palette and use a single cool glass interaction language. */
...
function stripAllEmoji(){var w=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);var n;while(n=w.nextNode()){if(n.parentElement&&n.parentElement.closest('script,style,textarea,pre,code'))continue;var v=n.nodeValue||'';var x=v.replace(emojiRegex(),'');if(x!==v)n.nodeValue=x}}function removePassiveNotices(){var enabled=(document.documentElement.getAttribute('data-cfnew-modules')||'').split(',').map(function(x){return x.trim()});if(enabled.indexOf('notices')>=0)return;document.querySelectorAll('.notice-banner,.announcement,.announcement-bar,.info-banner,.tip-banner,.toast-container,.snackbar-container').forEach(function(el){if(!el.closest('.modal,.dialog,[role="dialog"]'))el.remove()});document.querySelectorAll('p,div,span,small').forEach(function(el){if(el.children.length>4||el.closest('button,.modal,.dialog,[role="dialog"],.form-group,.field'))return;var t=stripEmoji(el.textContent);if(t.length>0&&t.length<420&&(/^(提示|温馨提示|公告|注意)[：:]/.test(t)||/国内测试 是由您梯子的/.test(t)||/发现新版本|当前版本已是最新版本|更新提示|版本更新|已有新版本|检查更新完成/.test(t))){var box=el.closest('.notice,.tip,.hint-box,.alert,.info-box,.description-box')||el;if(box&&!box.closest('.module-title,.section-title'))box.remove()}})}
...
function modules(){var enabled=(document.documentElement.getAttribute('data-cfnew-modules')||'').split(',').map(function(x){return x.trim()});if(enabled.indexOf('notifications')<0){var a=findSection('消息通知设置');if(a)a.remove()}if(enabled.indexOf('logs')<0){var b=findSection('查看操作日志');if(b)b.remove()}removePassiveNotices()}
function removeBottomActions(){document.querySelectorAll('footer,.footer,.social-links,.fixed-footer,.bottom-bar,.footer-actions').forEach(function(el){el.remove()});document.querySelectorAll('body *').forEach(function(el){if(el.id==='cfnewThemeSwitcher'||el.classList.contains('cfnew-float-actions'))return;var st;try{st=getComputedStyle(el)}catch(e){return}if(st.position!=='fixed'&&st.position!=='sticky')return;var r=el.getBoundingClientRect();if(r.bottom<innerHeight-90)return;var txt=stripEmoji(el.textContent);var links=el.querySelectorAll?el.querySelectorAll('a[href*="github"],a[href*="t.me"],a[href*="telegram"]'):[];if(links.length||/版本|更新日志|github|telegram/i.test(txt)){el.remove()}})}
...
function enhance(){if(RUN)return;RUN=true;try{retitle();modules();addSemanticIcons();stripAllEmoji();removeBottomActions();mountTheme();mountFloat();forceScroll();retitle()}finally{RUN=false}}
...
function __CFServerStripEmoji(value) { return String(value || '').replace(/[\p{Extended_Pictographic}\uFE0F\u200D]/gu, '').replace(/\s{2,}/g, ' '); }
...
function __CFServerNormalizeHtml(html) { return __CFServerIconizeHtml(__CFServerStripEmoji(html)); }
...
function __CFStyleRemoteHtml(html, kind = 'admin', modules = '') {
  ...
  html = html.replace(/edgetunnel\s*设置页面/gi, 'CFnew 设置中心');
		...
		if (访问路径 === 'version') {// 版本信息接口
			...
			if ((env.KV || env.C || env.c) && typeof (env.KV || env.C || env.c).get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					return __CFGetStyledStaticPage('/admin' + url.search, { modules: env.CFNEW_UI_MODULES || env.cfnew_ui_modules || '' });
