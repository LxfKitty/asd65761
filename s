...
var icons={globe:'<circle cx="12" cy="12" r="9"></circle><path d="M3 12h18M12 3a15 15 0 0 1 0 18M12 3a15 15 0 0 0 0 18"></path>',link:'<path d="M10.5 13.5l3-3"></path><path d="M7.2 15.8l-1 1a3.5 3.5 0 0 1-5-5l3-3a3.5 3.5 0 0 1 5 0"></path><path d="M16.8 8.2l1-1a3.5 3.5 0 0 1 5 5l-3 3a3.5 3.5 0 0 1-5 0"></path>',bolt:'<path d="M13 2L4.5 13h6L10 22l9.5-12h-6L13 2z"></path>',settings:'<circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.7 1.7 0 0 0 .3 1.9l.1.1-2.8 2.8-.1-.1a1.7 1.7 0 0 0-1.9-.3 1.7 1.7 0 0 0-1 1.6v.2h-4V21a1.7 1.7 0 0 0-1-1.6 1.7 1.7 0 0 0-1.9.3l-.1.1L4.2 17l.1-.1a1.7 1.7 0 0 0 .3-1.9A1.7 1.7 0 0 0 3 14H2.8v-4H3a1.7 1.7 0 0 0 1.6-1 1.7 1.7 0 0 0-.3-1.9L4.2 7 7 4.2l.1.1a1.7 1.7 0 0 0 1.9.3 1.7 1.7 0 0 0 1-1.6v-.2h4V3a1.7 1.7 0 0 0 1 1.6 1.7 1.7 0 0 0 1.9-.3l.1-.1L19.8 7l-.1.1a1.7 1.7 0 0 0-.3 1.9A1.7 1.7 0 0 0 21 10h.2v4H21a1.7 1.7 0 0 0-1.6 1z"></path>',lock:'<rect x="5" y="10" width="14" height="10" rx="3"></rect><path d="M8 10V7a4 4 0 0 1 8 0v3"></path>',cloud:'<path d="M7 18h10a4 4 0 0 0 .4-8 6 6 0 0 0-11.5-1.5A4.5 4.5 0 0 0 7 18z"></path>',refresh:'<path d="M20 6v5h-5"></path><path d="M4 18v-5h5"></path><path d="M18.5 9a7 7 0 0 0-11.7-2.6L4 11"></path><path d="M5.5 15a7 7 0 0 0 11.7 2.6L20 13"></path>',bell:'<path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9"></path><path d="M10 21h4"></path>',clipboard:'<rect x="5" y="4" width="14" height="17" rx="2"></rect><path d="M9 4V2h6v2M9 9h6M9 13h6"></path>',copy:'<rect x="8" y="8" width="11" height="11" rx="2"></rect><path d="M16 8V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h3"></path>',qr:'<rect x="3" y="3" width="6" height="6"></rect><rect x="15" y="3" width="6" height="6"></rect><rect x="3" y="15" width="6" height="6"></rect><path d="M15 15h2v2h-2zM19 15h2v6h-6v-2M15 19h2"></path>',save:'<path d="M5 3h12l2 2v16H5z"></path><path d="M8 3v6h8V3M8 21v-7h8v7"></path>',logout:'<path d="M10 5H5v14h5"></path><path d="M14 8l4 4-4 4M9 12h9"></path>',alert:'<path d="M12 3l10 18H2L12 3z"></path><path d="M12 9v5M12 17h.01"></path>',info:'<circle cx="12" cy="12" r="9"></circle><path d="M12 11v6M12 7h.01"></path>',route:'<circle cx="6" cy="18" r="2"></circle><circle cx="18" cy="6" r="2"></circle><path d="M8 18h3a3 3 0 0 0 3-3V9a3 3 0 0 1 3-3h1"></path>',shield:'<path d="M12 3l7 3v5c0 5-3 8-7 10-4-2-7-5-7-10V6l7-3z"></path>',spark:'<path d="M12 3l1.4 4.1L18 8.5l-4.1 1.4L12.5 14l-1.4-4.1L7 8.5l4.1-1.4L12 3z"></path><path d="M18 15l.8 2.2L21 18l-2.2.8L18 21l-.8-2.2L15 18l2.2-.8L18 15z"></path>',chevron:'<path d="M8 10l4 4 4-4"></path>'};
...
function iconFor(text){var t=(text||'').toLowerCase();if(/网络|地区|http|https|proxyip/.test(t))return'globe';if(/节点链接|复制|link/.test(t))return t.indexOf('复制')>=0?'copy':'link';if(/优选|闪|快速/.test(t))return'bolt';if(/详细|配置|参数|设置/.test(t))return'settings';if(/ech|加密|锁|socks/.test(t))return t.indexOf('socks')>=0?'shield':'lock';if(/cloudflare|cdn|云/.test(t))return'cloud';if(/订阅转换|刷新|重置|更新/.test(t))return'refresh';if(/消息|通知|telegram/.test(t))return'bell';if(/日志|清单|列表/.test(t))return'clipboard';if(/二维码/.test(t))return'qr';if(/保存/.test(t))return'save';if(/退出/.test(t))return'logout';if(/警告|注意|提示|危险/.test(t))return'alert';if(/路径|链式|路由/.test(t))return'route';return'info'}
...
function addSemanticIcons(){var sel='.module-title,.section-title,.card-title,.network-card-title,h2,h3,button,a.btn,button[class*="btn"]';document.querySelectorAll(sel).forEach(function(el){if(el.dataset.cfnewIconDone)return;var raw=stripEmoji(el.textContent);if(!raw)return;var should=/module-title|section-title|card-title|network-card-title/.test(el.className||'')||/二维码|复制|保存|退出|重置|参数配置|清除配置|路径模板|获取更多|为什么|开始优选|订阅接口|链式代理/.test(raw);if(!should)return;el.dataset.cfnewIconDone='1';el.insertAdjacentHTML('afterbegin',icon(iconFor(raw),'cfnew-inline-icon'))})}
...
function mountTheme(){if(document.getElementById('cfnewThemeSwitcher')){paint(get(),false);return}var brand=document.createElement('div');brand.className='cfnew-brand';brand.innerHTML='<span class="cfnew-brand-mark">'+icon('spark')+'</span><b>CFnew</b>';document.body.appendChild(brand);var box=document.createElement('div');box.id='cfnewThemeSwitcher';box.className='cfnew-theme-switcher';box.setAttribute('role','group');box.setAttribute('aria-label','主题模式');var cap=document.createElement('span');cap.textContent='主题';box.appendChild(cap);[['system','跟随系统'],['light','浅色'],['dark','深色']].forEach(function(x){var b=document.createElement('button');b.type='button';b.setAttribute('data-cfnew-mode',x[0]);b.textContent=x[1];b.onclick=function(){paint(x[0],true)};box.appendChild(b)});document.body.appendChild(box);paint(get(),false)}
...
function __CFStyleRemoteHtml(html, kind = 'admin', modules = '') {
  ...
  html = html.replace(/<head([^>]*)>/i, `<head$1>${__CF_UI_EARLY}${__CF_UI_CSS}`);
